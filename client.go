...
func NewClient(conf *ClientConfiguration) (mc *ModbusClient, err error) {
	...
	case "rtuovertcp":
		...
		if mc.conf.NetworkConnectionTimeout == 0 {
			mc.conf.NetworkConnectionTimeout = 5 * time.Second
		}
...
func (mc *ModbusClient) Open() (err error) {
	...
	case modbusRTUOverTCP:
		...
		sock, err = net.DialTimeout("tcp", mc.conf.URL, mc.conf.NetworkConnectionTimeout)
		if err != nil {
			return
		}
	...
	case modbusRTUOverUDP:
		...
		sock, err = net.DialTimeout("udp", mc.conf.URL, mc.conf.NetworkConnectionTimeout)
		if err != nil {
			return
		}
